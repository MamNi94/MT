...
class ObsListener(Node):
    def __init__(self, policy:nn.Module, arduino:serial.Serial):
        super().__init__('obs_listener')
        self.create_subscription(Float32MultiArray, '/lift/obs', self.callback, 10)
        self.policy = policy



    def callback(self, msg):
        # Convert list -> torch tensor (shape [obs_dim])
        action_deg = torch.tensor(msg.data, dtype=torch.float32)

        #with torch.no_grad():
           # action = self.policy(obs_tensor)

        #action_rad  = action.numpy()
        #action_deg = np.rad2deg(action_rad)

        print(action_deg[7:])

        phi_right = -action_deg[0] + 130
        phi_left = -action_deg[1] + 90
        phi_thumb = action_deg[2] + 80
        right = action_deg[3] + 110
        left = - action_deg[4] + 160
        thumb = action_deg[5] + 100

        print("Control Angles:", phi_right, phi_left, phi_thumb, right,left,thumb)

        # If you expect batched input (like in IsaacLab, shape [1, obs_dim]):
        #obs_tensor = obs_tensor.unsqueeze(0)

        

        #self.get_logger().info(f"Got obs tensor with shape {obs_tensor.shape}")

        self.get_logger().info(f"Action deg{action_deg[7:]}, Control Angles: {phi_right, phi_left, phi_thumb, right,left,thumb}")

     

def main():
    rclpy.init()
    node = ObsListener(policy, arduino)
    try:
        rclpy.spin(node)
    finally:
        node.destroy_node()
